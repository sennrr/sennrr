...
class Sollicitatie(db.Model):
    ...
    gebruiker = db.relationship('User', backref=db.backref('sollicitaties', lazy=True))
    vacature = db.relationship('Vacature', backref=db.backref('sollicitaties', lazy=True))
...
def dashboard():
    ...
    return render_template('dashboard.html', username=current_user.username, vacatures=vacatures, sollicitaties=sollicitaties)
...
def solliciteren(id):
    ...
    if request.method == 'POST':
        ...
        nieuwe_sollicitatie = Sollicitatie(user_id=current_user.id, vacature_id=id, motivatie=motivatie)
